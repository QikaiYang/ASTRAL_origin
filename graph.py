...
for i in range(len(NGEN)):
    for j in range(len(DLRT)):
        ...
        data_ = [error_astral, error_astrid]
        bplot = ax.boxplot(data_,  patch_artist=True)
        #----------------------------------------------------------
        for patch, color in zip(bplot['boxes'], colors):
            patch.set_facecolor(color)
        #----------------------------------------------------------
        ax.set_xticklabels(["ASTRAL","ASTRID"])
        ax.yaxis.grid(True)
        if(i==0):
            plt.ylabel(SIDE[j])
        if (j == 0):
            ax.set_title(TITLE[i])
plt.suptitle("Accuracies of ASTRAL and ASTRID on simulation data set when number of taxa = 100, sequence length = 25")
#------------------------------------------
...
for i in range(len(NGEN)):
    for j in range(len(DLRT)):
        ...
        if (j == 0):
            ax.set_title(TITLE[i])
plt.suptitle("Accuracies of ASTRAL and ASTRID on simulation data set when number of taxa = 100, sequence length = 50")
#------------------------------------------
...
for i in range(len(NGEN)):
    for j in range(len(DLRT)):
        ...
        if (j == 0):
            ax.set_title(TITLE[i])
plt.suptitle("Accuracies of ASTRAL and ASTRID on simulation data set when number of taxa = 100, sequence length = 100")
#------------------------------------------
...
plt.suptitle("Accuracies of ASTRAL and ASTRID on simulation data set when number of taxa = 100, sequence length = 250")
#------------------------------------------
plt.show()